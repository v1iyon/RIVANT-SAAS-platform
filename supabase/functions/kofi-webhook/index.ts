...
Deno.serve(async (req: Request) => {
  ...
  // ---------------------------------------------------------------
  // ФІКС (аудит #2, знахідка №10): ilike() трактує email як LIKE-патерн —
  // спецсимволи "%"/"_" у ньому не екранувались. Захопити чужий акаунт так
  // не можна (патерн все одно повинен збігтись із чиїмось РЕАЛЬНИМ email),
  // але кривий email із цими символами міг зламати активацію підписки
  // помилкою "multiple rows" (патерн випадково збігається з кількома
  // рядками одразу) замість чіткого not-found. .eq() з lowercase — точний
  // збіг без інтерпретації спецсимволів, той самий регістронезалежний
  // результат, бо email у БД зберігається в нижньому регістрі.
