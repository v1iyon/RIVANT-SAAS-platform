...
useEffect(() => {
  ...
  // ФІКС: раніше формула була "голою" (curr-prev)/prev*100 без жодних меж —
  // якщо попередній період був близьким до нуля (типово для нового/демо-
  // акаунта чи дня без продажів), результат вибухав до тисяч відсотків
  // (+7441%, +10441% і т.д. бачили на дашборді), хоча математично коректно.
  // Показувати таке користувачу нема сенсу — банально лякає й виглядає як
  // зламаний дашборд, хоча дані самі по собі вірні (проблема тільки в
  // ПОДАННІ %, не в revenue/profit/margin числах — їх не чіпаємо).
  //
  // Той самий принцип "з нуля відновились = зростання, а не нескінченність"
  // вже застосований у scripts/sync-stripe-core.mjs для алертів (рядок
  // ~637-639) — тут переносимо його ж ідею на картки кабінету:
  // - prev <= 0 (з нуля стартували) -> фіксовані +100% замість ділення на 0.
  // - Звичайна зміна -> межа ±100%.
         ...
         {activeView === "integrations" && (
            <div className="relative space-y-4 -mt-3 lg:-mt-4">
              {/* sticky (не absolute) — кружок має лишатись НА МІСЦІ ЕКРАНА поверх
                  ...
                  під нею не зсуваються вниз); -top-2 на ній — той самий візуальний
                  оверлап кута першої картки, що й у "шестерні" на Огляді
                  (absolute -top-2 -right-2), тільки тут sticky з тим самим
                  від'ємним offset-ом додатково тримає кружок у ЦІЙ ЖЕ точці
                  екрана, коли картки під ним прокручуються. z-30, щоб лишався
                  над заголовками карток, які він частково перекриває. */}
              <div className="sticky -top-2 z-30 h-0">
                <div className="flex justify-end">
                  <button
                    ...
                    className="-mr-2 w-7 h-7 rounded-full bg-gray-800/90 border border-gray-700 flex items-center justify-center text-gray-400 hover:text-white hover:bg-gray-700 transition-colors disabled:opacity-50 disabled:pointer-events-none"
