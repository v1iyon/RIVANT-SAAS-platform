...
// кабінетах є за роки.
//
// Тут — окремий крон, що раз на кілька хвилин бере ОДНУ інтеграцію з
// config.backfill_pending = true (виставляється в connect-integration,
// connect-stripe, google-ads-connect при (пере)підключенні) і тягне за неї
// до 365 днів історії тим самим sync-модулем, що й звичайний крон — просто
// з ширшим вікном (options.sinceDays). Обробка ПО ОДНІЙ інтеграції за
// прогін — навмисно: рік історії Stripe/Shopify/рекламного кабінету може
// бути тисячами записів, і краще кілька коротких безпечних прогонів під
// ліміт часу serverless-функції, ніж один довгий ризикований запит.
//
