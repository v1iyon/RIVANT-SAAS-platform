...
//
// ФІКС (аудит #4): раніше тут стояли ОБІ події — "charge.succeeded" і
// "payment_intent.succeeded". Для звичайного платежу карткою Stripe
// надсилає ОБИДВІ події на одну й ту саму оплату майже одночасно, а
// [businessId]/route.js обробляє кожен event незалежно і додає
// revenueDelta до metrics_computed.revenue щоразу — тобто кожен реальний
// платіж рахувався в живому лічильнику ДВІЧІ, поки годинний
// sync-stripe-core.mjs не перезапише рядок з нуля (reconciliation вже
// закладений в дизайн, але до наступного прогону дашборд/бот/алерти
// бачать завищену виручку). Лишаємо тільки "charge.succeeded" — це подія,
// яка реально відповідає надходженню грошей.
export const STRIPE_WEBHOOK_EVENTS = ["charge.succeeded"];
