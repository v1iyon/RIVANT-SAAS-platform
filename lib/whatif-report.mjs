...
async function fetchMetrics(businessId, sinceDate, untilDate) {
  let query = admin
...
async function fetchExpensesByChannel(businessId, sinceDate, untilDate) {
  let query = admin
    .from("expenses")
...
// Порівняння з попереднім місяцем — саме те, чого раніше в дайджесті не було
// взагалі, і через що він виглядав як "реконструкція, але коротша". Рахує
// сирі суми за попередній 30-денний період (без повного computeFacts —
// нам треба лише кілька підсумкових чисел для порівняння, не весь набір
// bestRevenueDay/lowestMarginDay тощо).
async function computeMonthOverMonth(businessId, currentSinceDate, currentFacts) {
  const prevUntil = currentSinceDate; // fetchMetrics(untilDate) ексклюзивний — попередній період не перетинається з поточним
  const prevSince = new Date(new Date(currentSinceDate).getTime() - 30 * 24 * 3600 * 1000).toISOString().slice(0, 10);

  const prevMetrics = await fetchMetrics(businessId, prevSince, prevUntil);
  if (prevMetrics.length < 5) return null; // недостатньо історії "до" — новий бізнес, порівнювати нема з чим

  const prevExpenses = await fetchExpensesByChannel(businessId, prevSince, prevUntil);
  const prevRevenue = prevMetrics.reduce((s, m) => s + Number(m.revenue), 0);
  const prevCost = prevMetrics.reduce((s, m) => s + Number(m.cost), 0);
  const prevOrders = prevMetrics.reduce((s, m) => s + Number(m.orders), 0);
  const prevAvgMargin = prevMetrics.reduce((s, m) => s + Number(m.margin_pct), 0) / prevMetrics.length;
  const prevTopChannel = Object.entries(prevExpenses).sort((a, b) => b[1] - a[1])[0];

  const pctChange = (curr, prev) => (prev > 0 ? Number((((curr - prev) / prev) * 100).toFixed(1)) : null);

  return {
    periodStart: prevMetrics[0].date,
    periodEnd: prevMetrics[prevMetrics.length - 1].date,
    revenueChangePct: pctChange(currentFacts.totalRevenue, prevRevenue),
    ordersChangePct: pctChange(currentFacts.totalOrders, prevOrders),
    costChangePct: pctChange(currentFacts.totalCost, prevCost),
    marginChangePp: Number((currentFacts.avgMarginPct - prevAvgMargin).toFixed(1)),
    topChannelChanged: Boolean(prevTopChannel && currentFacts.topChannel && prevTopChannel[0] !== currentFacts.topChannel.name),
    previousTopChannel: prevTopChannel ? prevTopChannel[0] : null,
  };
}
...
async function buildReport(businessId, mode) {
  ...
  // Порівняння з попереднім місяцем має сенс тільки для дайджесту — у
  // реконструкції (12 міс) "попередній період" це вже 2 роки тому, не
  // релевантно для того самого звіту.
  const monthOverMonth = mode === "monthly_digest" ? await computeMonthOverMonth(businessId, sinceDate, facts) : null;
  ...
  return { ok: true, facts, narrative, language, metrics, expensesBySource, monthOverMonth };
}
