...
async function listAccessibleCustomers(accessToken) {
  ...
  if (!res.ok) {
    ...
    const errorCode = nestedErrors[0]?.errorCode ? JSON.stringify(nestedErrors[0].errorCode) : null;
    const message = nestedMessage || topMessage || res.status;
    console.error("listAccessibleCustomers failed:", JSON.stringify({ topMessage, nestedMessage, errorCode, raw: data }));
