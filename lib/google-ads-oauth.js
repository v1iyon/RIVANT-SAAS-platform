...
async function listAccessibleCustomers(accessToken) {
  ...
  const ids = (data.resourceNames || []).map((rn) => rn.split("/")[1]).filter(Boolean);
  console.log("DEBUG listAccessibleCustomers ->", JSON.stringify(ids));
  return ids;
}
...
async function queryCustomerClients(accessToken, developerToken, seedCustomerId) {
  ...
  if (!res.ok) {
    ...
    console.log(`DEBUG queryCustomerClients seed=${seedCustomerId} FAILED ->`, JSON.stringify(data));
    return [];
  }
  return data.results || [];
}
...
async function listAdAccounts(accessToken) {
  ...
  for (const seedId of seedIds) {
    ...
    console.log(`DEBUG queryCustomerClients seed=${seedId} -> ${rows.length} row(s)`, JSON.stringify(rows));
  ...
  console.log("DEBUG listAdAccounts raw accounts before dedup ->", JSON.stringify(accounts));

  // Дедуплікація — той самий кінцевий акаунт міг прийти через кілька seed
  // (наприклад, якщо користувач має прямий доступ І доступ через MCC
  // одночасно — саме такий випадок і зламав синк раніше). listAccessibleCustomers
  // НЕ гарантує порядок, тому не можна просто брати "перше попадання": якщо
  // прямий доступ (без менеджера) прийде першим, ми запам'ятаємо
  // loginCustomerId: null для акаунта, який насправді працює ТІЛЬКИ через MCC
  // (типовий випадок для developer token, прив'язаного до конкретного
  // Manager Account) — і синк впаде з "The caller does not have permission",
  // хоча по факту правильний login-customer-id для цього ж акаунта був у
  // списку, просто під іншим seed.
  //
  // Тому дедуплікуємо через Map і ВІДДАЄМО ПЕРЕВАГУ запису з непустим
  // loginCustomerId, якщо для одного customerId прийшло кілька варіантів.
  const byCustomerId = new Map();
  for (const a of accounts) {
    const existing = byCustomerId.get(a.customerId);
    if (!existing || (!existing.loginCustomerId && a.loginCustomerId)) {
      byCustomerId.set(a.customerId, a);
    }
  }
