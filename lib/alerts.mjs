...
async function getTeamContacts(businessId) {
  ...
  return (members || []).map((m) => m.telegram_id);
}

// Обгортка над sendAlert: шле власнику (як і раніше) + fan-out усім активним
// учасникам команди. Дедуп у sendAlert працює по business_id+type, тому
// повторний виклик для команди нічого не задублює в alerts_log — просто
// довідсилаємо Telegram-повідомлення додатковим chat_id, без повторного insert.
async function sendAlertToBusiness(businessId, ownerContact, alertParams) {
  ...
  const teamIds = await getTeamContacts(businessId);
  if (!teamIds.length) return delivered;
...
async function sendDailyReport(businessId, businessName, contact, kind, stats) {
  ...
  const teamIds = await getTeamContacts(businessId);
  if (teamIds.length) {
    await Promise.all(teamIds.map((chatId) => sendTelegram(chatId, message)));
  }
}
