...
// Обгортка над sendAlert: шле власнику (як і раніше) + fan-out учасникам
// команди, у яких у categories є категорія цього конкретного алерту.
// Дедуп у sendAlert працює по business_id+type, тому повторний виклик для
// команди нічого не задублює в alerts_log — просто довідсилаємо
// Telegram-повідомлення додатковим chat_id, без повторного insert.
//
// Якщо sendAlert() не відправив (спрацював дедуп по type за останні 24
// години), значить власнику це сповіщення щойно йшло — команду теж не
// дублюємо. Без цієї перевірки учасники команди отримували алерт заново на
// кожному прогоні синку (раз в годину), поки проблема не вирішена, хоча
// власник захищений дедупом — саме це і було причиною "спаму" на
// підключеному акаунті.
async function sendAlertToBusiness(businessId, ownerContact, alertParams) {
  ...
  const recipients = members.filter((m) => (m.categories || ALERT_CATEGORIES).includes(category));
  if (!recipients.length) return delivered;
  ...
  await Promise.all(recipients.map((m) => sendTelegram(m.telegram_id, fullMessage)));
  return delivered;
}
