...
// ВАЖНО (фикс "уведомления не приходят, хотя всё вроде отправлялось"):
// раньше здесь не проверялся res.ok — если Telegram API отвечал ошибкой
// (403 "bot was blocked by the user", 400 "chat not found", протухший
// BOT_TOKEN и т.п.), fetch() всё равно резолвился без исключения, и
// вызывающий код (sendAlert, sendDailyReport) считал сообщение
// доставленным. В частности scripts/daily-reports.mjs после этого писал
// дедуп-запись "звіт вже надіслано сьогодні" — и следующий часовой прогон
// уже не пытался повторить отправку, хотя фактически ничего не дошло.
// Теперь функция возвращает true/false и логирует причину сбоя, чтобы
// вызывающий код мог принять решение (не отмечать как "отправлено", если
// разослать не удалось ни по одному каналу).
async function sendTelegram(chatId, text) {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      await logError({
        source: "telegram",
        message: `sendMessage failed: ${res.status}`,
    ...
    console.error(`Telegram send threw for chat ${chatId}:`, err.message);
    await logError({ source: "telegram", message: "sendMessage threw", details: err.message });
    return false;
  }
}
...
async function sendEmail(to, subject, text) {
  ...
  try {
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({ from: "RIVANT <hello@rivant-os.com>", to, subject, text }),
    });
    if (!res.ok) {
      const body = await res.text().catch(() => "");
      console.error(`Email send failed for ${to}: ${res.status} — ${body}`);
      await logError({
        source: "email",
        message: `Resend send failed: ${res.status}`,
        details: body.slice(0, 500),
      });
      return false;
    }
    ...
    await logError({ source: "email", message: "Resend send threw", details: err.message });
    return false;
  }
}
...
async function sendAlert({ businessId, type, severity, message, aiExplanation, userLang, telegramId, email, emailEnabled, cooldownHours = 24 }) {
  ...
  // Запись в alerts_log (Risks tab) уже сделана выше и остаётся в силе
  // независимо от того, дошло ли сообщение в Telegram/email — это
  // реальное событие, пользователь должен увидеть его хотя бы в кабинете.
  // А вот факт "разослали ли куда-то вообще" отдельно логируем, чтобы
  // не терять такие сбои молча.
  let anyChannelDelivered = false;
  if (telegramId) anyChannelDelivered = (await sendTelegram(telegramId, fullMessage)) || anyChannelDelivered;
  if (emailEnabled && email) anyChannelDelivered = (await sendEmail(email, "RIVANT Alert", fullMessage)) || anyChannelDelivered;
  if ((telegramId || (emailEnabled && email)) && !anyChannelDelivered) {
    console.error(`Alert ${type} for business ${businessId} was logged but NOT delivered to any channel`);
  }
  return true;
}
...
//
// ВАЖНО: возвращает { delivered: boolean } — true, только если сообщение
// РЕАЛЬНО ушло хотя бы по одному каналу владельца (Telegram или email).
// Раньше функция ничего не возвращала, и scripts/daily-reports.mjs после
// вызова безусловно писал дедуп-запись "отправлено сегодня" — из-за этого
// сбой доставки в Telegram (заблокированный бот, протухший токен и т.п.)
// навсегда "съедал" отчёт за день: повторных попыток уже не было, а по
// кабинету это выглядело так, будто всё отправилось нормально. Теперь
// вызывающий код сам решает, писать ли дедуп — см. daily-reports.mjs.
async function sendDailyReport(businessId, businessName, contact, kind, stats) {
  ...
  if (!delivered) {
    console.error(`Daily report (${kind}) for business ${businessId} was NOT delivered to owner on any channel`);
  }
  ...
  // Доставка команді НЕ впливає на delivered/дедуп власника — це окремі
  // отримувачі, їхні збої не повинні блокувати повтор для власника.
  const members = await getTeamContacts(businessId);
