...
export async function runDailyReports() {
  ...
  for (const business of businesses) {
    try {
      ...
      // Раніше перевірка була "рівно hour === 8 / === 20" — якщо конкретний
      // годинний прогін GitHub Actions запізнювався (затримки крону там —
      // звичайна справа, іноді 15-60+ хв, а буває й пропуск прогону
      // цілком) і "вилітав" за межі потрібної години, звіт за цей день
      // пропускався НАЗАВЖДИ (дедуп нижче захищає лише від дублів, не
      // компенсує пропуск). Тепер це вікно в кілька годин: якщо перший
      // прогін всередині вікна не спрацював, наступний годинний прогін
      // всередині того самого вікна ще встигне надіслати звіт (дедуп по
